...
public class SalesEngineWorkflowBoundedStates {

    private static Logger LOGGER = LoggerFactory.getLogger(SalesEngineWorkflowBoundedStates.class);
    ...
    public static void main(String[] args) throws IOException {
        ...
        JavaPairRDD<String, Tuple2<List<SalesRow>, CustomerRow>> customersSalesJoinRDD = salesDf.join(customerDf, partitioner);

        final JavaPairRDD<DimensionRecord, Double> stateSalesPairRDD = customersSalesJoinRDD.flatMapToPair(CustomerSalesToStateSalesFunction.transform());

        final JavaPairRDD<DimensionRecord, Double> stateSalesPairReducedRDD = stateSalesPairRDD.reduceByKey((s1, s2) -> s1 + s2, 1);
